            ...
            case 'data-url':
              value = (
                <a
                  ...
                  href={ "/uploads/"+SubmissionInfo.formData[current]}
                >
              ...
              switch (jsonschema.properties[current].items.type) {
                case 'string':
                  switch (jsonschema.properties[current].items.format) {
                    case 'data-url':
                      return <a target="_blank" className="db link pa3 mv1 shadow-4 navy underline-hover overflow-hidden" href={"/uploads/"+item} key={i}>{item}</a>;
                    default:
                      return item;
                  }
                default:
